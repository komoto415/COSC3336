...
def main():
	if len(sys.argv) > 3 or len(sys.argv) < 2:
		print('Usage: python aetg.py num_factors [num_levels]')
	...
	some_data = what_is_this(the_set)
...
appendToRow = lambda row, val: row.append(val)
...
def create_m_set(start_level):
	the_set = []
    # The code has been removed D:
	return the_set


# What is this ???
# Tested
def what_is_this(the_array):
		all_pairs = []
...
def potential_pairs(the_row, dictionary):
	...
	# 			count += 1
	validDict = lambda row, dictionary: True if dictionary[row] == False else False
	countThis = list(filter(lambda x: validDict(x, dictionary), the_row))
	return len(countThis)
...
def add_next(the_row, the_test, dictionary):
	...
	greaterList = list(filter(lambda x: x > max, scores))
	equalList = list(filter(lambda y: y == max, scores))
